...
const WidgetForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/whatsapp/widgets', data, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      console.log('Backend Response:', response.data);
      setWidgetCode(response.data.embedCode);
      setWidgetId(response.data.widgetId);
      setGreetingImageUrl(response.data.greetingImageUrl || ''); 
      setError('');
  ...
  useEffect(() => {
    if (widgetId && isPreviewVisible) {
      ...
      script.src = `http://localhost:5000/api/whatsapp/widget.js?widgetId=${widgetId}`;
      script.async = true;
